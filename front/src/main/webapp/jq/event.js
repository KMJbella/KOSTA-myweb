//자바스크립트(js/event.js) -> 제이쿼리 문법으로 바꿔보기
...
$(function(){
    ...
    //------------계산기 START---------
    //DOM트리에서 class 속성값이 calculator인
    //객체의 자식중 모든 button객체찾기

    //querySelectorAll()의 반환형은 Nodelist자료형
    //$()의 반환형은 jquery객체형
    //$()의 결과가 여러 객체인 경우 각 인덱스의 요소는 js객체이다
    //js객체를 jq객체로 변환하려면 $(js객체)처리 필요하다
    //반복문용 jq메서드는 each()
    let $btArr = $('div.calculator>button'); //
    // $btArr[0].hide(); //괄호를 사용하여 아래처럼 jq로 바꾸기
    // $($btArr[0]).hide();
    ...
    // alert($cb.prop('checkbox'));  //checked프로퍼티값 얻기
